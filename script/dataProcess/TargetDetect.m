...
%%%%%%%%%%%%%%%%%%%%%%%%%
left=Img(:,1:max(1,winsize));
right=Img(:,max(1,(col-winsize+1)):col);
top=Img(1:max(1,winsize),:);
bottom=Img(max(1,(row-winsize+1)):row,:);
...
DetRS(:,1:max(1,winsize))=0;
DetRS(:,max(1,(col-winsize+1)):col)=0;
DetRS(1:max(1,winsize),:)=0;
DetRS(max(1,(row-winsize+1)):row,:)=0;
%�ڶ�����ֵ�ָ�
for i=1:row
    for j=1:col
        if DetRS(i,j)>0
            l=max(1, i-localsize);
            r=min(row, i+localsize);
            t=max(1, j-localsize);
...
% 自适应形态学结构元素半径：round(4 * min(row,col) / 128)
se_radius = max(1, round(4 * min(row, col) / 128));
